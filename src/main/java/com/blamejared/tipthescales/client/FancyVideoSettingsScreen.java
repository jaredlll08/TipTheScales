...
public class FancyVideoSettingsScreen extends VideoSettingsScreen {
    ...
    protected void init() {
        ...
        for(int i = 0; i < OPTIONS.length; i++) {
            ...
            if(option.translatedBaseMessage.getString()
                    .equals(AbstractOption.GUI_SCALE.translatedBaseMessage.getString())) {
                OPTIONS[i] = new SliderPercentageOption("options.guiScale", 0, Minecraft.getInstance()
                        .getMainWindow()
                        .calcGuiScale(0, Minecraft.getInstance()
                                .getForceUnicodeFont()), 1, gameSettings1 -> (double) gameSettings1.guiScale, (gameSettings1, aDouble) -> {
                    gameSettings1.guiScale = aDouble.intValue();
    ...
    public boolean superMouseReleased(double mouseX, double mouseY, int buttonId) {
        ...
        return this.getEventListenerForPos(mouseX, mouseY)
                .filter(listener -> listener.mouseReleased(mouseX, mouseY, buttonId))
                .isPresent();
    }
